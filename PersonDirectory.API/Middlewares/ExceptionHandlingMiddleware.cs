...
namespace PersonDirectory.API.Middlewares
{
    public class ExceptionHandlingMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            {
                await HandleExceptionAsync(context, StatusCodes.Status404NotFound, _localizer[$"{ex.Message}", ex.Key], ex);
            }
            catch (AlreadyExsistExeption ex)
            {
                await HandleExceptionAsync(context, StatusCodes.Status400BadRequest, _localizer[$"{ex.Message}", ex.Key], ex);
            }
            catch (BadRequestExeption ex)
            {
                await HandleExceptionAsync(context, StatusCodes.Status400BadRequest, _localizer[$"{ex.Message}"], ex);
            }
            catch (Exception ex)
            {
